...
- **AI 深度分析 (AI Cortex)**: 对模糊案例使用本地 LLM 进行自然语言推理，弥补规则引擎的刚性不足。- **自适应性 (Adaptability)**: 能够对未知的攻击手段做出反应。s-Immunity
...
│         Local LLM for deep analysis of ambiguous cases       │- **危险信号理论 (Danger Theory)**: 通过监测系统压力信号（CPU 飙升、进程洪水等）动态调整免疫灵敏度。
...
│  │   Receptor    │  │   Immune     │  │    Effector       │  │

│  │   感知层      │  │   决策层      │  │    响应层          │  │- **行为免疫 (Behavioral Immunity)**: 基于行为模式而非静态特征来检测威胁。

│  │              │  │              │  │                   │  │

│  │ TLR (进程监控)│→│ Memory B Cell│→│ Cytotoxic T Cell  │  │## 🏗 架构设计 (Architecture)- **自适应性 (Adaptability)**: 能够对未知的攻击手段做出反应。

│  │ Hash Cache   │  │ Path Valid.  │  │ Quarantine        │  │
...
### 1. 感知层 (Sensors / Innate Immunity)│                    AI Cortex (大脑皮层)                       │### 1. 感知层 (Sensors / Innate Immunity)

*对应生物学：皮肤、Toll 样受体*

- **TollLikeReceptor**: 进程快照 + 差分扫描，检测新进程│         Local LLM for deep analysis of ambiguous cases       │*对应生物学：皮肤、巨噬细胞*

- **HashCache**: LRU 哈希缓存，避免重复计算 SHA256（补体调理素标记）

- **SafeHandle**: RAII 包装器，确保 Windows 句柄安全释放│                   (Ollama + qwen2.5:3b)                      │- **功能**: 负责实时监控系统状态。



### 2. 决策层 (Core / Adaptive Immunity)├─────────────────────────────────────────────────────────────┤- **技术**: 

*对应生物学：T细胞、B细胞、淋巴结*

- **Memory B Cell**: SHA256 哈希白名单持久化（免疫记忆）│                                                              │    - Windows ETW (Event Tracing for Windows)

- **PathValidator (MHC)**: 路径验证，检测进程名伪装（如 svchost.exe 出现在非 System32 目录）

- **DangerTheoryEngine**: 系统压力信号监测（CPU/内存/进程洪水），动态调整免疫灵敏度│  ┌──────────────┐  ┌──────────────┐  ┌───────────────────┐  │    - File System Minifilter (文件变动监控)

- **YARA Engine**: 已知恶意软件特征匹配（抗原数据库，12+ 条规则）

│  │   Receptor    │  │   Immune     │  │    Effector       │  │    - Process Callbacks (进程创建/销毁监控)

### 3. AI 决策中枢 (AI Cortex / Cerebral Cortex)

*对应生物学：大脑皮层*│  │   感知层      │  │   决策层      │  │    响应层          │  │

- **AiCortex**: 通过 Ollama 调用本地 LLM，对模糊案例进行深度分析

- **设计原则**: 本地优先、优雅降级、非阻塞、可审计│  │              │  │              │  │                   │  │### 2. 决策层 (Core / Adaptive Immunity)



### 4. 响应层 (Effectors)│  │ TLR (进程监控)│→│ Memory B Cell│→│ Cytotoxic T Cell  │  │*对应生物学：T细胞、B细胞、淋巴结*

*对应生物学：抗体中和、细胞吞噬、淋巴结隔离*

- **CytotoxicTCell**: 终止恶意进程（细胞凋亡），RAII 安全句柄管理│  │ Hash Cache   │  │ Path Valid.  │  │ Quarantine        │  │- **功能**: 分析感知层传来的数据，判断是否为威胁。

- **Quarantine**: 文件隔离管理（淋巴结隔离），支持释放/删除

- **分级响应**: Log → Monitor → Quarantine → Terminate│  │              │  │ Danger Theory│  │                   │  │- **模块**:



### 5. 日志系统 (Logger / Immunological Memory)│  │              │  │ YARA Engine  │  │                   │  │    - **Negative Selection (负向选择)**: 过滤掉已知的“自身”进程（白名单）。

- JSONL 结构化日志，便于 SIEM 集成

- 自动轮转（50MB/文件）│  └──────────────┘  └──────────────┘  └───────────────────┘  │    - **Danger Theory (危险理论)**: 监测系统压力的信号（如CPU突然飙升、大量文件被重命名）。
...
- **关键库**:│              Structured JSONL event logging                   │*对应生物学：抗体中和、细胞吞噬*

    - `windows-rs` (WinAPI)

    - `yara-x` (特征匹配)└─────────────────────────────────────────────────────────────┘- **功能**: 对确认为威胁的目标采取行动。
...
## 🚀 快速开始 (Quick Start)### 1. 感知层 (Sensors / Innate Immunity)    - **Terminate (杀伤)**: 结束恶意进程。



```bash*对应生物学：皮肤、Toll 样受体*    - **Alert (炎症反应)**: 通知用户。

# 1. 安装 Ollama (可选，用于 AI 深度分析)

# 下载: https://ollama.ai- **TollLikeReceptor**: 进程快照 + 差分扫描，检测新进程
...
cargo run -- --mode learn### 2. 决策层 (Core / Adaptive Immunity)- **操作系统**: Windows (首选目标平台)



# 4. 启动被动监控*对应生物学：T细胞、B细胞、淋巴结*- **关键库**: 
...
# 6. 查看隔离区- **DangerTheoryEngine**: 系统压力信号监测（CPU/内存/进程洪水），动态调整免疫灵敏度
...
| `--mode` | `monitor` | 运行模式: `learn`, `monitor`, `active`, `quarantine-list` |- **AiCortex**: 通过 Ollama 调用本地 LLM，对模糊案例进行深度分析- [ ] 实现基础的进程监控 (Process Monitor)

| `--policy` | - | 响应策略: `log`, `kill`, `quarantine` |

| `--ai` | `true` | 是否启用 AI Cortex |- **设计原则**: 本地优先、优雅降级、非阻塞、可审计- [ ] 实现简单的“白名单”机制
...
新进程 ──→ YARA扫描(黑名单) ──→ 路径验证(MHC) ──→ 哈希白名单(B细胞记忆)- **CytotoxicTCell**: 终止恶意进程（细胞凋亡），RAII 安全句柄管理- [ ] 实现基础的行为分析 (例如：检测勒索软件诱饵文件的修改)
        ...
        匹配=CRITICAL          伪装=CRITICAL          可信=SAFE- **Quarantine**: 文件隔离管理（淋巴结隔离），支持释放/删除
