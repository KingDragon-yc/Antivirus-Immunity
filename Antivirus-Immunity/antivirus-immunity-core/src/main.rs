...
use immune::{ImmuneSystem, Assessment, DangerTheoryEngine};
...
use logger::{Logger, SecurityEvent, EventType};
...
async fn main() -> anyhow::Result<()> {
    ...
    let logger = Logger::new().unwrap_or_else(|e| {
        eprintln!("[!] Failed to initialize logger: {}. Continuing without file logging.", e);
        Logger::new().unwrap() // retry
    });
    
    // Determine Policy
    let active_defense = args.mode == "active" || args.policy.as_deref() == Some("kill") || args.policy.as_deref() == Some("quarantine");
    ...
    if quarantine_list_mode {
        ...
        if let Some(ref q) = quarantine {
            ...
            if active.is_empty() {
                ...
                println!("{:<38} {:<30} {:<20} {}", "ID", "ORIGINAL PATH", "PROCESS", "DATE");
                println!("{:-<38} {:-<30} {:-<20} {:-<20}", "", "", "", "");
                for entry in active {
                    println!("{:<38} {:<30} {:<20} {}",
                        entry.id,
    ...
    logger.log(&SecurityEvent {
        ...
        process_path: None,
        assessment: None,
        detail: format!("System started in '{}' mode with '{}' policy", args.mode, policy_str),
        action_taken: None,
        ai_verdict: None,
        danger_level: None,
    });
        ...
        match receptor.scan_diff() {
            Ok((new_procs, _dead_pids)) => {
                for p in new_procs {
                    ...
                    println!("{:<6} {:<25} {:<15} {:.60}", p.pid, p.name, status_str, info);
                    ...
                    if needs_ai && ai_cortex.is_available() {
                        let ctx = ProcessContext {
                            ...
                            path: p.path.clone(),
                            hash: p.hash.clone(),
                            path_verdict: immune_system.get_path_verdict_string(
                                &p.name,
                                p.path.as_deref(),
                            ),
                            yara_matches: p.path.as_deref()
                                .map(|path| immune_system.get_yara_matches(path))
                                .unwrap_or_default(),
                            danger_level: format!("{:?}", current_danger),
                            is_known_hash: p.hash.as_deref()
                        ...
                        match ai_cortex.evaluate(&ctx).await {
                            Some(verdict) => {
                                println!("[{}] (confidence: {:.0}%)",
                                    verdict.classification, verdict.confidence * 100.0);
                                println!("       Reasoning: {}", 
                                ...
                                logger.log(&SecurityEvent {
                                    ...
                                    ai_verdict: Some(serde_json::to_string(&verdict).unwrap_or_default()),
                                ...
                                if active_defense && verdict.recommendation == "TERMINATE" {
                                    ...
                                    match CytotoxicTCell::induce_apoptosis(p.pid) {
                                        Ok(_) => {
                                            ...
                                            logger.log_action(p.pid, &p.name, "TERMINATE", "AI-directed elimination");
                                        }
                                    ...
                                    if let Some(ref mut q) = quarantine {
                                        if let Some(ref path) = p.path {
                                            ...
                                            match q.isolate(path, p.hash.clone(), &verdict.reasoning, &p.name, p.pid) {
                                                Ok(entry) => {
                                                    println!("ISOLATED (ID: {})", &entry.id[..8]);
                                                    logger.log_action(p.pid, &p.name, "QUARANTINE", &format!("Isolated: {}", entry.id));
                                                }
                                                Err(e) => println!("FAILED: {}", e),
                                            }
                                        }
